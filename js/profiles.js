...
class QuinielaStorage {
  ...
  async initData() {
    ...
    let matchesFromAPI = [];
    try {
      const response = await fetch("api/2026.json?v=5");
  ...
  async getDuels() {
    ...
    try {
      ...
      console.error("Error getting duels:", e);
      return [];
    }
  }
  ...
  async sendDuel(matchId, challengerId, defenderId) {
    ...
    try {
      await this.db.collection("groups").doc(groupId).collection("duels").doc(duelId).set({
        matchId: matchId,
      ...
      console.error("Error sending duel:", e);
      return false;
    }
  }
  ...
  async updateDuelStatus(duelId, status) {
    ...
    try {
      ...
      console.error("Error updating duel:", e);
      return false;
    }
  }
}
