...
def is_exact_match(preds_raw, match_id, matches):
    if not preds_raw: return False
    m = next((x for x in matches if x["id"] == match_id), None)
    if not m or m.get("realHomeScore") is None or m.get("realAwayScore") is None: return False
    
    if "stringValue" in preds_raw:
        parts = preds_raw["stringValue"].split(",")
        idx = int(match_id.replace("m", "")) - 1
        if idx < len(parts) and parts[idx] and parts[idx] != "-":
            try:
                ph, pa = map(int, parts[idx].split("-"))
                rh, ra = int(m["realHomeScore"]), int(m["realAwayScore"])
...
def fetch_and_push():
    ...
    # ---------------------------------------------------------
    try:
        future_url = "https://site.api.espn.com/apis/site/v2/sports/soccer/fifa.world/scoreboard?dates=20260704-20260719"
    ...
    ESPNDictionary = {
        ...
        "Round of 16 1 Winner": "Ganador 16vos 1", "Round of 16 2 Winner": "Ganador 16vos 2",
        "Round of 16 3 Winner": "Ganador 16vos 3", "Round of 16 4 Winner": "Ganador 16vos 4",
        "Round of 16 5 Winner": "Ganador 16vos 5", "Round of 16 6 Winner": "Ganador 16vos 6",
        "Round of 16 7 Winner": "Ganador 16vos 7", "Round of 16 8 Winner": "Ganador 16vos 8"
    }
    
    # ---------------------------------------------------------
    # ---------------------------------------------------------
    future_slots = ["m89", "m91", "m93", "m95", "m90", "m92", "m94", "m96", "m97", "m99", "m98", "m100", "m101", "m102", "m103", "m104"]
        ...
        try:
            ...
            for group in groups:
                ...
                try:
                    ...
                    # Pre-fetch all predictions for the group
                    group_preds = {}
                    for prof in profiles:
                        prof_id = prof["name"].split("/")[-1]
                        try:
                            pred_req = urllib.request.Request(BASE_URL + "/groups/" + group_name + "/predictions/" + prof_id)
                            pred_doc = json.loads(urllib.request.urlopen(pred_req).read().decode())
                            group_preds[prof_id] = pred_doc.get("fields", {}).get("data", {})
                        except:
                            group_preds[prof_id] = {}
                    ...
                    duels = []
                    try:
                        duels_req = urllib.request.Request(BASE_URL + "/groups/" + group_name + "/duels?pageSize=1000")
                        duels_docs = json.loads(urllib.request.urlopen(duels_req).read().decode()).get("documents", [])
                        for d in duels_docs:
                            d_fields = d.get("fields", {})
                            if d_fields.get("status", {}).get("stringValue", "") == "accepted":
                                duels.append({
                                    "matchId": d_fields.get("matchId", {}).get("stringValue", ""),
                                    "challengerId": d_fields.get("challengerId", {}).get("stringValue", ""),
                                    "defenderId": d_fields.get("defenderId", {}).get("stringValue", "")
                                })
                    except Exception as e:
                        pass # No duels subcollection yet

                    for prof in profiles:
                        prof_id = prof["name"].split("/")[-1]
                        prof_fields = prof.get("fields", {})
                        
                        base_pts = calculate_points(group_preds.get(prof_id, {}), local_data["matches"])
                        
                        duel_pts = 0
                        for d in duels:
                            match_id = d["matchId"]
                            if prof_id == d["challengerId"]:
                                opp_id = d["defenderId"]
                                my_exact = is_exact_match(group_preds.get(prof_id, {}), match_id, local_data["matches"])
                                opp_exact = is_exact_match(group_preds.get(opp_id, {}), match_id, local_data["matches"])
                                if my_exact and not opp_exact: duel_pts += 3
                                elif opp_exact and not my_exact: duel_pts -= 3
                            elif prof_id == d["defenderId"]:
                                opp_id = d["challengerId"]
                                my_exact = is_exact_match(group_preds.get(prof_id, {}), match_id, local_data["matches"])
                                opp_exact = is_exact_match(group_preds.get(opp_id, {}), match_id, local_data["matches"])
                                if my_exact and not opp_exact: duel_pts += 3
                                elif opp_exact and not my_exact: duel_pts -= 3
                                
                        total_pts = base_pts + duel_pts
                        
                        old_pts = int(prof_fields.get("points", {}).get("integerValue", "0"))
                        
                        if old_pts != total_pts:
                            patch_prof_url = BASE_URL + "/groups/" + group_name + "/profiles/" + prof_id + "?updateMask.fieldPaths=points"
                            prof_fields["points"] = {"integerValue": str(total_pts)}
                            ...
                            print(f"   -> Recalculated {prof_id} points: {old_pts} -> {total_pts} (Base: {base_pts}, Duels: {duel_pts})")
